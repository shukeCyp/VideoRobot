...
class JimengImg2VideoTaskManager:
    ...
    async def start_task(self, image_path: str, prompt: str = "", account_id: int = None, headless: bool = True) -> Dict[str, Any]:
        """启动图生视频任务"""
        import uuid
        task_id = str(uuid.uuid4())
        
        logger.info(f"启动图生视频任务: {task_id}")
        ...
        # 异步执行任务
        asyncio.create_task(
            self.execute_task(task_id, image_path, prompt, account_id, headless)
        )
